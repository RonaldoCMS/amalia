...
export default function PublicCvPage() {
  ...
  useEffect(() => {
    if (!username) return
    new CvService().getPublicByUsername(username as string)
      .then(setSession)
  ...
  if (notFound || !session?.cvData) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#0a0a0f] gap-4">
        <p className="text-zinc-500 font-mono text-sm">CV non trovato</p>
        <Link href="/cv" className="text-violet-400 font-mono text-sm hover:underline">← Galleria CV</Link>
      </div>
    )
  }
        ...
        <div className="relative z-10 max-w-2xl mx-auto w-full px-4 py-8 flex-1 space-y-6">
          ...
          {cvData.skills.length > 0 && (
            <div className="border border-zinc-800 rounded-xl bg-zinc-900/40 p-5">
              <h2 className="text-[10px] font-mono uppercase tracking-widest text-zinc-600 mb-4">Skill tecniche</h2>
              ...
              <p className="text-[10px] font-mono text-zinc-600 mt-3">
                <span className="text-violet-400">✓</span> = verificato da sfide di programmazione reali su Amalia
              </p>
            </div>
          )}
          ...
          {cvData.softSkills && cvData.softSkills.length > 0 && (
            <div className="border border-zinc-800 rounded-xl bg-zinc-900/40 p-5">
              <h2 className="text-[10px] font-mono uppercase tracking-widest text-zinc-600 mb-4">Soft Skills</h2>
              <div className="flex flex-wrap gap-2">
          ...
          {cvData.education && (
            <div className="border border-zinc-800 rounded-xl bg-zinc-900/40 p-5">
              <h2 className="text-[10px] font-mono uppercase tracking-widest text-zinc-600 mb-3">Formazione</h2>
          ...
          {cvData.projects.length > 0 && (
            <div className="border border-zinc-800 rounded-xl bg-zinc-900/40 p-5">
              <h2 className="text-[10px] font-mono uppercase tracking-widest text-zinc-600 mb-4">Progetti</h2>
          ...
          {cvData.experience.length > 0 && (
            <div className="border border-zinc-800 rounded-xl bg-zinc-900/40 p-5">
              <h2 className="text-[10px] font-mono uppercase tracking-widest text-zinc-600 mb-4">Esperienze</h2>
              <div className="space-y-4">
          ...
          {amaliaStats && (
            <div className="border border-violet-400/20 rounded-xl bg-violet-400/5 p-5">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-[10px] font-mono uppercase tracking-widest text-violet-400">Statistiche Amalia Platform</h2>
              </div>

              <div className="grid grid-cols-3 gap-3 mb-4">
                <div className="text-center">
                  <p className="text-xl font-bold font-mono text-cyan-400">{amaliaStats.challengesCompleted}</p>
                  <p className="text-[9px] font-mono text-zinc-600 uppercase tracking-wider">sfide</p>
                </div>
                <div className="text-center">
                  <p className="text-xl font-bold font-mono text-emerald-400">{amaliaStats.accuracy}%</p>
                  <p className="text-[9px] font-mono text-zinc-600 uppercase tracking-wider">accuratezza</p>
                </div>
                <div className="text-center">
                  <p className="text-xl font-bold font-mono text-amber-400">{amaliaStats.totalScore}</p>
                  <p className="text-[9px] font-mono text-zinc-600 uppercase tracking-wider">punti</p>
                </div>
              </div>
          ...
          <div className="border border-zinc-800 rounded-xl bg-zinc-900/20 p-5 text-center">
            ...
            <p className="text-[10px] font-mono text-zinc-600 mt-1">
              La piattaforma di sfide di programmazione · amalia.dev
            </p>
            {publicUrl && (
