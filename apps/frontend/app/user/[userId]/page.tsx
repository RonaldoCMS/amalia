...
export default function UserProfilePage() {
  ...
  const achievements = useMemo(() => {
    ...
    if (correctCount >= 1)  list.push({ icon: '🚀', label: t('achievementFirstStep'),    desc: t('achievementFirstStepDesc') })
    if (correctCount >= 10) list.push({ icon: '⚔️', label: t('achievementCodeWarrior'),   desc: t('achievementCodeWarriorDesc') })
    if (correctCount >= 50) list.push({ icon: '🧠', label: t('achievementMaster'),         desc: t('achievementMasterDesc') })
    if (correctCount >= 100) list.push({ icon: '🌟', label: t('achievementLegend'),       desc: t('achievementLegendDesc') })
    if (totalScore >= 500)  list.push({ icon: '💎', label: t('achievementDiamond'),         desc: t('achievementDiamondDesc') })
    if (totalScore >= 2000) list.push({ icon: '👑', label: t('achievementGrandmaster'),     desc: t('achievementGrandmasterDesc') })
    if (total >= 5 && accuracy >= 0.8) list.push({ icon: '🎯', label: t('achievementSniper'), desc: t('achievementSniperDesc') })
    if (total >= 20 && accuracy >= 0.9) list.push({ icon: '⚡', label: t('achievementLightning'), desc: t('achievementLightningDesc') })
    if (totalDuels >= 1)    list.push({ icon: '🥊', label: t('achievementDuelist'),        desc: t('achievementDuelistDesc') })
    if (wins >= 5)          list.push({ icon: '🏅', label: t('achievementWarrior'),       desc: t('achievementWarriorDesc') })
    if (wins >= 20)         list.push({ icon: '🏆', label: t('achievementChampion'),        desc: t('achievementChampionDesc') })
    if (totalDuels >= 10 && winRate >= 60) list.push({ icon: '🔥', label: t('achievementInvincible'), desc: t('achievementInvincibleDesc') })
    if (posts.length >= 1)  list.push({ icon: '✍️', label: t('achievementWriter'),          desc: t('achievementWriterDesc') })
    if (posts.length >= 10) list.push({ icon: '📢', label: t('achievementInfluencer'),      desc: t('achievementInfluencerDesc') })
    return list
  }, [profile, posts])
  ...
  const handleRemove = async () => {
    if (!friendStatus?.friendshipId || !confirm(t('removeFriendConfirm'))) return
    setFriendLoading(true)
    try {
  ...
  const tabs: { id: Tab; label: string }[] = [
    { id: 'overview', label: t('tabProfile') },
    { id: 'challenge', label: t('tabChallenge') },
    { id: 'duelli', label: t('tabDuels') },
    { id: 'post', label: `${t('tabPosts')}${posts.length > 0 ? ` (${posts.length})` : ''}` },
  ]

  const memberSince = new Date(profile.createdAt).toLocaleDateString(LOCALE_DATE_MAP[locale] || 'it-IT', { month: 'long', year: 'numeric' })
        ...
        <div className="border border-zinc-800 rounded-xl bg-zinc-900/40 p-5">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] font-mono text-zinc-600 mt-2">{t('memberSince')} {memberSince}</p>
            </div>
          </div>
          ...
          <div className="flex gap-4 mt-4 pt-4 border-t border-zinc-800/60">
            <div className="text-center">
              ...
              <p className="text-[10px] font-mono text-zinc-600">{t('statPoints')}</p>
            </div>
            <div className="text-center">
              <p className="text-base font-bold font-mono text-emerald-400">{cs.correctCount}</p>
              <p className="text-[10px] font-mono text-zinc-600">{t('statCorrect')}</p>
            </div>
            <div className="text-center">
              <p className="text-base font-bold font-mono text-amber-400">{ds.wins}</p>
              <p className="text-[10px] font-mono text-zinc-600">{t('statWins')}</p>
            </div>
            <div className="text-center">
              <p className="text-base font-bold font-mono text-violet-400">{posts.length}</p>
              <p className="text-[10px] font-mono text-zinc-600">{t('statPosts')}</p>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-5">
            ...
            {profile.languages.length > 0 && (
              <div className="border border-zinc-800 rounded-xl bg-zinc-900/40 p-4">
                <h3 className="text-[10px] font-mono uppercase tracking-widest text-zinc-600 mb-3">{t('languages')}</h3>
            ...
            {profile.goals.length > 0 && (
              <div className="border border-zinc-800 rounded-xl bg-zinc-900/40 p-4">
                <h3 className="text-[10px] font-mono uppercase tracking-widest text-zinc-600 mb-3">{t('goals')}</h3>
                <div className="flex flex-wrap gap-1.5">
                  {profile.goals.map(g => (
            ...
            {profile.githubUrl && (
              <div className="border border-zinc-800 rounded-xl bg-zinc-900/40 p-4">
                <h3 className="text-[10px] font-mono uppercase tracking-widest text-zinc-600 mb-3">{t('links')}</h3>
            ...
            {achievements.length > 0 && (
              <div className="border border-zinc-800 rounded-xl bg-zinc-900/40 p-4">
                <h3 className="text-[10px] font-mono uppercase tracking-widest text-zinc-600 mb-3">{t('achievements')}</h3>
        ...
        {activeTab === 'challenge' && (
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-3 gap-3">
              <StatBlock label={t('challengePoints')} value={cs.totalScore} color="text-cyan-400" />
              <StatBlock label={t('challengeCorrect')} value={cs.correctCount} color="text-emerald-400" />
              <StatBlock label={t('challengeAccuracy')} value={`${accuracy}%`} color="text-violet-400" />
            </div>

            {/* Accuracy bar */}
            <div className="border border-zinc-800 rounded-xl bg-zinc-900/40 p-4 space-y-3">
              <h3 className="text-[10px] font-mono uppercase tracking-widest text-zinc-600">{t('accuracyLabel')}</h3>
              ...
              <div className="flex gap-4 text-xs font-mono">
                <span className="text-emerald-400">✓ {cs.correctCount} {t('correctLabel')}</span>
                <span className="text-red-400">✗ {cs.wrongCount} {t('wrongLabel')}</span>
              </div>
            </div>
            ...
            {byLanguage.length > 0 && (
              <div className="border border-zinc-800 rounded-xl bg-zinc-900/40 p-4">
                <h3 className="text-[10px] font-mono uppercase tracking-widest text-zinc-600 mb-3">{t('byLanguage')}</h3>
            ...
            {byType.length > 0 && (
              <div className="border border-zinc-800 rounded-xl bg-zinc-900/40 p-4">
                <h3 className="text-[10px] font-mono uppercase tracking-widest text-zinc-600 mb-3">{t('byType')}</h3>
                <div className="grid grid-cols-2 gap-2">
            ...
            {byLevel.length > 0 && (
              <div className="border border-zinc-800 rounded-xl bg-zinc-900/40 p-4">
                <h3 className="text-[10px] font-mono uppercase tracking-widest text-zinc-600 mb-3">{t('byDifficulty')}</h3>
            ...
            {history.length > 0 && (
              <div className="border border-zinc-800 rounded-xl bg-zinc-900/40 p-4">
                <h3 className="text-[10px] font-mono uppercase tracking-widest text-zinc-600 mb-3">{t('latestChallenges')}</h3>
                <div className="space-y-2">
            ...
            {!historyLoading && history.length === 0 && (
              <p className="text-zinc-600 text-sm text-center py-8">{t('noChallenges')}</p>
            )}
          </div>
        )}
        ...
        {activeTab === 'duelli' && (
          <div className="space-y-5">
            <div className="grid grid-cols-2 gap-3">
              <StatBlock label={t('duelTotal')} value={ds.totalDuels} color="text-zinc-300" />
              <StatBlock label={t('duelWins')} value={ds.wins} color="text-amber-400" />
              <StatBlock label={t('duelLosses')} value={ds.losses} color="text-red-400" />
              <StatBlock label={t('duelWinRate')} value={`${ds.winRate}%`} color="text-cyan-400" />
            </div>

            {/* W/L/D visual bar */}
            {ds.totalDuels > 0 && (
              <div className="border border-zinc-800 rounded-xl bg-zinc-900/40 p-4">
                <h3 className="text-[10px] font-mono uppercase tracking-widest text-zinc-600 mb-3">{t('duelDistribution')}</h3>
                ...
                <div className="flex gap-4 mt-3 text-xs font-mono">
                  <span className="text-amber-400">🏆 {ds.wins} {t('duelWon')}</span>
                  {(ds.totalDuels - ds.wins - ds.losses) > 0 && (
                    <span className="text-zinc-400">⚖️ {ds.totalDuels - ds.wins - ds.losses} {t('duelDrawn')}</span>
                  )}
                  <span className="text-red-400">💀 {ds.losses} {t('duelLost')}</span>
                </div>
              </div>
            )}
            ...
            {ds.totalDuels > 0 && (
              <div className="border border-zinc-800 rounded-xl bg-zinc-900/40 p-4 flex items-center gap-6">
                ...
                <div>
                  <p className="text-xs font-mono text-zinc-400">{t('globalWinRate')}</p>
            ...
            {ds.totalDuels === 0 && (
              <p className="text-zinc-600 text-sm text-center py-8">{t('noDuels')}</p>
            )}
          </div>
        )}
        ...
        {activeTab === 'post' && (
          <div>
            ...
            {!postsLoading && posts.length === 0 && (
              <p className="text-zinc-600 text-sm text-center py-8">{t('noPosts')}</p>
            )}
