...
const contractOptions: { value: ContractType; label: string }[] = [
  { value: ContractType.Permanent, label: 'Indeterminato' },
  { value: ContractType.FixedTerm, label: 'Determinato' },
...
const workModeOptions: { value: WorkMode; label: string }[] = [
  ...
  { value: WorkMode.Hybrid, label: 'Ibrido' },
  { value: WorkMode.Onsite, label: 'In sede' },
]
...
export default function CreateOfferPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (!sector.trim()) return setError('Inserisci il settore')
  ...
  return (
    <div className="min-h-screen bg-grid flex flex-col relative">
      ...
      <main className="relative z-10 flex-1 max-w-2xl w-full mx-auto px-4 py-8 pb-24 sm:pb-8">
        ...
        <h1 className="text-lg font-semibold text-zinc-100 font-mono mb-6">📝 pubblica offerta</h1>

        <div className="flex flex-col gap-5">
          {/* Title */}
          <div>
            <label className="text-xs text-zinc-500 font-mono mb-1.5 block">titolo *</label>
            <input value={title} onChange={e => setTitle(e.target.value)} placeholder="es. Senior React Developer"
          ...
          {/* Description */}
          <div>
            <label className="text-xs text-zinc-500 font-mono mb-1.5 block">descrizione *</label>
          ...
          {/* Sector */}
          <div>
            <label className="text-xs text-zinc-500 font-mono mb-1.5 block">settore *</label>
            <input value={sector} onChange={e => setSector(e.target.value)} placeholder="es. Fintech, E-commerce, SaaS"
              className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2.5 text-sm text-zinc-100 font-mono placeholder-zinc-700 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/20" />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-zinc-500 font-mono mb-1.5 block">tipo contratto</label>
            ...
            <div>
              <label className="text-xs text-zinc-500 font-mono mb-1.5 block">modalità</label>
              <div className="flex flex-wrap gap-1.5">
          ...
          {/* Location */}
          <div>
            <label className="text-xs text-zinc-500 font-mono mb-1.5 block">sede (opzionale)</label>
            <input value={location} onChange={e => setLocation(e.target.value)} placeholder="es. Milano, Roma"
              className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2.5 text-sm text-zinc-100 font-mono placeholder-zinc-700 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/20" />
          </div>

          {/* Salary range */}
          <div>
            <label className="text-xs text-zinc-500 font-mono mb-1.5 block">RAL (k€, opzionale)</label>
            <div className="grid grid-cols-2 gap-3">
              <input value={salaryMin} onChange={e => setSalaryMin(e.target.value.replace(/\D/g, ''))} placeholder="min"
                className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2.5 text-sm text-zinc-100 font-mono placeholder-zinc-700 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/20" />
              <input value={salaryMax} onChange={e => setSalaryMax(e.target.value.replace(/\D/g, ''))} placeholder="max"
                className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2.5 text-sm text-zinc-100 font-mono placeholder-zinc-700 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/20" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs text-zinc-500 font-mono mb-1.5 block">anni esperienza</label>
              <input value={yearsRequired} onChange={e => setYearsRequired(e.target.value.replace(/\D/g, ''))}
                className="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2.5 text-sm text-zinc-100 font-mono placeholder-zinc-700 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/20" />
            </div>
            <div>
              <label className="text-xs text-zinc-500 font-mono mb-1.5 block">scadenza (giorni)</label>
          ...
          <div className="border border-zinc-800 rounded-xl p-5 bg-zinc-900/30">
            <label className="text-xs text-zinc-500 font-mono uppercase tracking-widest mb-3 block">hard skills richieste</label>
            <div className="flex gap-2 mb-3">
              <input value={newSkill} onChange={e => setNewSkill(e.target.value)} placeholder="es. react, python..."
          ...
          {/* Soft skills */}
          <div className="border border-zinc-800 rounded-xl p-5 bg-zinc-900/30">
            <label className="text-xs text-zinc-500 font-mono uppercase tracking-widest mb-3 block">soft skills (opzionale)</label>
            <div className="flex gap-2 mb-3">
              <input value={newSoft} onChange={e => setNewSoft(e.target.value)} placeholder="es. team working, comunicazione..."
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            <button onClick={handleSubmit} disabled={submitting}
              ...
              {submitting ? 'pubblicazione...' : '🚀 pubblica offerta'}
            </button>
          </div>
        </div>
      </main>
